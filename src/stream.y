...
#include <string.h>
#include <stdio.h>
#include "stream.tab.h"  /* Include generated header for type definitions */

/* Types are declared in %code requires and included via stream.tab.h */
...
int stream_yy_parse(void *scanner);
int stream_yy_drive_lex(void *yylval_param, void *yyloc_param, void *scanner);
int stream_yy_drive_parse(void *scanner);
void stream_yy_error(void *yylloc, void *scanner, const char *s);

#define stream_yy_lex stream_yy_drive_lex
...
int stream_yy_drive_parse(void *scanner) {
    return yaml_processor_drive_parse(stream_yy_parse, scanner);
}

static void stream_yy_recover_error(void *scanner, const char *msg) {
    yaml_processor_raise_error(stream_yy_error, scanner, msg);
}

#define RECOVER(msg) do { stream_yy_recover_error(scanner, (msg)); } while (0)
#define RECOVER_SYNC(msg) do { stream_yy_recover_error(scanner, (msg)); yyerrok; } while (0)
%}
...
/* Directives */
%token YAML_DIRECTIVE TAG_DIRECTIVE
